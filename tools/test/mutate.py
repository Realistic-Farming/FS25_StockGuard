...
MUTATIONS = [
 ...
 # ── review round 2 (Bob re-review BLOCKER, ledger d1529a1) ─────────────
 ("R15-claim-guard-removed", OPS,
  "if carrier ~= nil and live ~= nil and claimed[carrier.carrierId] then",
  "if false then", 1,
  "a later saved stock takes over or resets a carrier already claimed this load (Bob's guard-removal check)"),
 ("R16-reattach-does-not-claim", OPS,
  ...
  "        elseif carrier ~= nil and live ~= nil and self.stocks[s.stockId] == nil and nativeAmount == s.observedAmount and SGValues.equal(nativeMaterial, s.materialRef) then\n", 1,
  "a reattach leaves the carrier open for an older history row"),
 ("R17-mismatch-does-not-claim", OPS,
  "            claimed[carrier.carrierId] = true\n            live.knowledge = \"UNKNOWN\"",
  "            live.knowledge = \"UNKNOWN\"", 1,
  "a mismatch leaves the carrier open for an older history row"),
 ("H19-added-vehicle-bypasses-queue", NH,
  "        if binding ~= nil then self:markDirty(self.nativeLease, binding, false) end\n    end\n    self:flush()\nend",
  '        if binding ~= nil then self.handle.refreshCarrier(self.nativeLease, binding, "VEHICLE_ADDED") end\n    end\nend', 1,
 ...
 ("H20-added-storage-bypasses-queue", NH,
  ...
  '        self.handle.refreshCarrier(self.nativeLease, A.storageBinding(placeable, slot, name), "STORAGE_ADDED")\n    end', 1,
 ...
 # A16, A17, A19 and A21 carry the fill-unit kind's own neighbouring line since SG2-3a,
 # whose Combine buffer kind repeats each of these lines.
 ("A16-layout-unchecked", NA,
  'if d.configFileName ~= nil and d.configFileName ~= current then return nil, "LAYOUT_CHANGED" end\n        if A.fillUnitBindingFor(vehicle, d.fillUnitIndex)',
  '\n        if A.fillUnitBindingFor(vehicle, d.fillUnitIndex)', 1,
 ...
 ("A21-fillunit-enumerate-server-gate", NA,
  ...
  "        if false then return {} end\n        local out = {}\n        for _, vehicle in ipairs(vehicles and vehicles() or {}) do\n            local fu = vehicle.spec_fillUnit", 1,
 ...
 ("H3-no-coalescing", NH,
  "self:markDirty(self.nativeLease, binding, boundary)\nend\n\n--- A FillUnit observer report",
  "self:markDirty(self.nativeLease, binding, true)\nend\n\n--- A FillUnit observer report", 1,
 ...
 ("H15-fillunit-no-coalescing", NH,
  "self:markDirty(self.nativeLease, binding, boundary)\nend\n\nfunction H:observeVehicle",
  "self:markDirty(self.nativeLease, binding, true)\nend\n\nfunction H:observeVehicle", 1,
