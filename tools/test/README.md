# FS25_StockGuard self-test suite
...
| **Logic** | `run-tests.mjs` (`fengari` Lua VM) | Behavioural regressions in pure logic - material records, settlement, save envelopes, scoped delivery - by loading the real `src` modules against a mocked FS25 environment and asserting. |

Logic suites are added per member as the delivery's reference bars are ported.
